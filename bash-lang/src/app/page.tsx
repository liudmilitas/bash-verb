...
export default function Home() {
  ...
  return (
    <main
      className={
        "bg-gradient-to-r from-emerald-500 to-lime-600 flex flex-col justify-around items-center lg:px-16 md:px-12 sm:px-6 px-4 h-full min-h-screen " +
        styles.pattern
      }
    >
      <section className="w-full h-full flex flex-col content-center items-center justify-center">
        <h1 className="text-2xl lg:text-3xl text-white p-4 font-bold drop-shadow-lg w-full max-w-[800px] text-center lg:text-left md:text-left">
          Спрягай-ка!💡
        </h1>
        <div className="bg-white h-full rounded shadow-2xl w-full max-w-[800px] flex flex-col justify-between items-center">
          ...
          <form className="flex flex-col p-4 h-full">
            ...
            <button
              className="bg-lime-700 hover:bg-lime-800 duration-300 text-white shadow p-2 m-4 rounded-lg"
              type="submit"
            >
              Проверить
            </button>
      ...
      <div className="flex flex-col items-center justify-center text-white p-2 mt-8 drop-shadow-lg fill-white">
        <p className=" text-white p-1 drop-shadow w-full text-center font-semibold">
          Спрягай-ка! Тренируем башкирскую грамматику
        </p>
        <div className="flex">
          <Link
            target={"_blank"}
            className="flex mx-2 hover:fill-emerald-100 font-semibold"
            href="https://github.com/liudmilitas/bash-lang"
          >
            <svg
              className="w-6 h-6 fill-white hover:fill-emerald-100"
              viewBox="0 0 64 64"
            >
          ...
          <span className="font-semibold">liudmilitas, 2023</span>
        </div>
      </div>
    </main>
  );
